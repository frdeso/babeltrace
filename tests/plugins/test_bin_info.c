...
static uint32_t opt_debug_link_crc;
...
{
	{ "foo-addr", 0, 0, G_OPTION_ARG_INT64, &opt_func_foo_addr, "Offset to printf in foo", "0xX" },
	{ "printf-offset", 0, 0, G_OPTION_ARG_INT64, &opt_func_foo_printf_offset, "Offset to printf in foo", "0xX" },
	{ "printf-lineno", 0, 0, G_OPTION_ARG_INT64, &opt_func_foo_printf_line_no, "Line number to printf in foo", "N" },
	{ "tp-offset", 0, 0, G_OPTION_ARG_INT64, &opt_func_foo_tp_offset, "Offset to tp in foo", "0xX" },
	{ "tp-lineno", 0, 0, G_OPTION_ARG_INT64, &opt_func_foo_tp_line_no, "Line number to tp in foo", "N" },
	{ "debug-link-crc", 0, 0, G_OPTION_ARG_INT64, &opt_debug_link_crc, "Debug link CRC", "0xX" },
	{ "build-id", 0, 0, G_OPTION_ARG_STRING, &opt_build_id, "Build ID", "XXXXXXXXXXXXXXX" },
	{ "debug-info-dir", 0, 0, G_OPTION_ARG_STRING, &opt_debug_info_dir, "Debug info directory", NULL },
	{ NULL }
};
...
void subtest_lookup_function_name(struct bin_info *bin, uint64_t addr, char *func_name)
{
	...
	if (_func_name) {
		ok(strcmp(_func_name, func_name) == 0,
			"bin_info_lookup_function_name - correct function name (%s == %s)", func_name, _func_name);
		free(_func_name);
		_func_name = NULL;
	} else {
		skip(1, "bin_info_lookup_function_name - function name is NULL");
	}

	/* Test function name lookup - erroneous address */
	ret = bin_info_lookup_function_name(bin, SO_INV_ADDR, &_func_name);
	ok(ret == -1 && _func_name == NULL, "bin_info_lookup_function_name - fail on invalid addr");
	if (_func_name) {
		free(_func_name);
	}
}
...
static
void subtest_lookup_source_location(struct bin_info *bin, uint64_t addr, uint64_t line_no, char *filename)
{
	int ret;
	struct source_location *src_loc = NULL;

	ret = bin_info_lookup_source_location(bin, addr, &src_loc);
	ok(ret == 0, "bin_info_lookup_source_location successful at 0x%x", addr);
	if (src_loc) {
		ok(src_loc->line_no == line_no,
			"bin_info_lookup_source_location - correct line_no (%d == %d)", line_no, src_loc->line_no);
		ok(strcmp(src_loc->filename, filename) == 0,
			"bin_info_lookup_source_location - correct filename (%s == %s)", filename, src_loc->filename);
	...
	ok(ret == -1 && src_loc == NULL, "bin_info_lookup_source_location - fail on invalid addr");
	if (src_loc) {
		source_location_destroy(src_loc);
	}
}
...
void test_bin_info_build_id(const char *bin_info_dir)
{
	...
	ret = bin_info_set_build_id(bin, invalid, 2);
	...
	subtest_lookup_function_name(bin, func_foo_printf_addr, func_foo_printf_name);

	/* Test source location lookup */
	subtest_lookup_source_location(bin, func_foo_printf_addr, opt_func_foo_printf_line_no, FUNC_FOO_FILENAME);
...
void test_bin_info_bundled(const char *bin_info_dir)
{
	...
	subtest_lookup_source_location(bin, func_foo_tp_addr, opt_func_foo_tp_line_no, FUNC_FOO_FILENAME);
